...
# =====================================================================
# 3. FILTRO FONDAMENTALE APERTO (SENZA PALETTI SETTORIALI)
# =====================================================================
def verifica_salute_finanziaria_intelligente(t_obj, cmf_val):
  try:
    ...
    # Filtro flessibile: esclude solo crolli totali di fatturato se non c'è supporto smart money
...
def analizza_titolo(ticker_str, data_oggi):
  try:
    ...
    # 2. Salute finanziaria flessibile (senza barriere settoriali)
    ...
    # --- NUOVI VERDETTI DIRETTI E SPICCIOLI ---
...
def invia_email_report(file_path, data_oggi, num_titoli):
  ...
  body = f'Ciao! Il tuo consulente virtuale ha completato l\'analisi in data {data_oggi}.\n\nSono state filtrate le migliori occasioni di mercato senza barriere di settore, con storno >= 15% e flussi istituzionali.\nApri il file Excel allegato per leggere i verdetti operativi.\n\nBuon gain!'
